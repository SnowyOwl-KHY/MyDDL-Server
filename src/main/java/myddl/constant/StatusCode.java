...
public class StatusCode {
    public static final int OK = 200; //- 成功返回状态，对应，GET,PUT,PATCH,DELETE.
    public static final int CREATED = 201; //created  - 成功创建。
    public static final int NOT_MODIFIED = 304; //Not Modified – 客户端使用缓存数据
    public static final int BAD_REQUEST = 400; //Bad Request – 请求无效，参数错误。需要附加细节解释如 "JSON无效"
    public static final int UNAUTHORIZED = 401; //Unauthorized – 未授权,请求需要用户验证
    public static final int FORBIDDEN = 403; //Forbidden – 服务器已经理解了请求，但是拒绝服务或这种请求的访问是不允许的。
    public static final int NOT_FOUND = 404; //Not found – 没有发现该资源
    ...
    public static final int UNPROCESSABLE_ENTITY = 422; //Unprocessable Entity – 只有服务器不能处理实体时使用，比如图像不能被格式化，或者重要字段丢失。
    public static final int TOO_MANY_REQUEST = 429; //too many request - 请求过多。
    public static final int INTERNAL_SERVER_ERROR = 500; //Internal Server Error - 服务器内部异常。
...
//    public static final int OK=200; //一切正常，对GET和POST请求的应答文档跟在后面。
//    public static final int Created=201; //服务器已经创建了文档，Location头给出了它的URL。
//    public static final int Accepted = 202;//已经接受请求，但处理尚未完成。
//    public static final int Non_Authoritative_Information=203;//文档已经正常地返回，但一些应答头可能不正确，因为使用的是文档的拷贝，非权威性信息（HTTP 1.1新）。
//    public static final int No_Content=204;//没有新文档，浏览器应该继续显示原来的文档。如果用户定期地刷新页面，而Servlet可以确定用户文档足够新，这个状态代码是很有用的。
//    public static final int Reset_Content=205;//没有新的内容，但浏览器应该重置它所显示的内容。用来强制浏览器清除表单输入内容（HTTP 1.1新）。
//    public static final int Partial_Content=206;//客户发送了一个带有Range头的GET请求，服务器完成了它（HTTP 1.1新）。
//
//    //3xx - 重定向
//    public static final int Multiple_Choices=300;//客户请求的文档可以在多个位置找到，这些位置已经在返回的文档内列出。如果服务器要提出优先选择，则应该在Location应答头指明。
